...
const DEFAULT_FORM = {
  ...
  irrigationType: "Diario", imageUrl: "",
};
...
// Imágenes sugeridas por nombre de planta
const QUICK_IMAGES = [
...
function PlantModal({ isOpen, onClose, onSave, plant, defaultSector = "Superior" }) {
  ...
  useEffect(() => {
    if (isOpen) {
      setFormData(plant ? { ...DEFAULT_FORM, ...plant } : { ...DEFAULT_FORM, sector: defaultSector });
      setErrors({});
      setStep(1);
          ...
          <div style={{ padding:"28px 28px 0", display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
            <div style={{ display:"flex", alignItems:"center", gap:14 }}>
              <div style={{
                ...
                {/* Preview imagen si hay URL válida */}
                {formData.imageUrl && imgPreviewOk
                  ? <img src={formData.imageUrl} alt="" style={{ width:"100%", height:"100%", objectFit:"cover", borderRadius:13 }} />
                  : (plant ? "✏️" : "🌱")}
              </div>
              <div>
                ...
                <div style={{ fontSize:12, color:"#78909c" }}>
                  {step === 1 ? "Información básica" : "Humedad y configuración"}
                </div>
              </div>
            </div>
            ...
            <AnimatePresence mode="wait">
                  ...
                  <div style={{ marginBottom:20 }}>
                    ...
                    {formData.imageUrl && (
                      <motion.div initial={{ opacity:0, height:0 }} animate={{ opacity:1, height:"auto" }} style={{ marginTop:8 }}>
                        ...
                        />
                        {!imgPreviewOk && (
                          <div style={{ fontSize:11, color:"#f87171", marginTop:4 }}>⚠ URL de imagen inválida</div>
                        )}
                      </motion.div>
                    )}
                  </div>
              ...
              {step === 2 && (
                ...
                >
                  ...
                  {/* Preview rango */}
                  <div style={{ padding:16, borderRadius:14, background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", marginBottom:24 }}>
                    ...
                    <div style={{ position:"relative", height:10, background:"rgba(255,255,255,0.06)", borderRadius:99, overflow:"hidden", marginBottom:8 }}>
                      {rangeValid && (
                        <div style={{
                          position:"absolute", top:0, bottom:0, left:`${minH}%`, width:`${maxH-minH}%`,
                          background:"linear-gradient(90deg,#34d399,#60a5fa)", borderRadius:99,
                          boxShadow:"0 0 12px rgba(52,211,153,0.4)", transition:"all 0.4s ease",
                        }} />
                      )}
                    </div>
