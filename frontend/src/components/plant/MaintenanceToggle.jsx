...
export default function MaintenanceToggle({ plant, onUpdate }) {
  ...
  const toggle = async () => {
    ...
    try {
      ...
      setShowNote(false);
      setNote("");
      onUpdate?.(res.data.plant);
    } catch (err) {
      setError(err.mensajeUsuario || err.response?.data?.error || "Error al cambiar modo mantenimiento");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ width: "100%" }}>
      <button
        ...
        style={{
          ...
          padding: "11px 14px",
          borderRadius: 12,
          border: isActive ? "1.5px solid rgba(251,191,36,0.45)" : "1px solid rgba(255,255,255,0.09)",
      ...
      >
        {loading ? (
          ...
          <>🔧 Mantenimiento activo - tocar para desactivar</>
        ) : (
