...
function Login({ onLogin }) {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      await api.post("/api/auth/register", { name: rName, username: rUser, email: rEmail, password: rPass });
  ...
  const handleForgot = async (e) => {
    ...
    try {
      await api.post("/api/auth/forgot-password", { email: fEmail });
  ...
  return (
    <div className="lp-page">
      ...
      {/* ══ PANEL IZQUIERDO — con animación de entrada ══ */}
      ...
      {/* ══ PANEL DERECHO — con animación de entrada ══ */}
            ...
            <AnimatePresence mode="wait">
              ...
              {tab === "login" && (
                <motion.form key="login" onSubmit={handleLogin}
                  ...
                  <Field label="Usuario" icon={<FaUser />} placeholder="Tu nombre de usuario"
                    value={lUser} onChange={e => setLUser(e.target.value)} error={lErr.user} autoComplete="username" />
                  <Field label="Contraseña" icon={<FaLock />} type={showP ? "text" : "password"}
                    placeholder="Tu contraseña" value={lPass} onChange={e => setLPass(e.target.value)}
              ...
              {tab === "register" && (
                <motion.form key="register" onSubmit={handleRegister}
                  ...
                  <Field label="Usuario" icon={<FaUser />} placeholder="ej: david_riego"
                    value={rUser} onChange={e => setRUser(e.target.value)} error={rErr.user} autoComplete="username" />
                  <Field label="Email" icon={<FaEnvelope />} type="email" placeholder="tu@email.com"
                    value={rEmail} onChange={e => setREmail(e.target.value)} error={rErr.email} autoComplete="email" />
                  <Field label="Contraseña" icon={<FaLock />} type={showP ? "text" : "password"}
                  ...
                  <Field label="Confirmar contraseña" icon={<FaLock />} type={showP2 ? "text" : "password"}
                    placeholder="Repite tu contraseña" value={rConfirm} onChange={e => setRConfirm(e.target.value)}
              ...
              {tab === "forgot" && (
                <motion.form key="forgot" onSubmit={handleForgot}
                  ...
                  <Field label="Email registrado" icon={<FaEnvelope />} type="email" placeholder="tu@email.com"
                    value={fEmail} onChange={e => setFEmail(e.target.value)} error={fErr} autoComplete="email" />
                  <button className="lp-submit-btn" type="submit" disabled={loading}>
                    {loading && <span className="spinner" />}
